...
test("can create scalar functions", function () {
  const db = new DB();
...
test("can delete scalar functions", function () {
  const db = new DB();
  const answer = () => 42;
...
test("specify function names", function () {
  const db = new DB();
...
test("can't define the same function twice", function () {
  const db = new DB();
  const test = () => {};
  db.createFunction(test);
  assertThrows(() => db.createFunction(test), (err) => {
...
test("can throw errors in user defined functions", function () {
  const db = new DB();
  const error = (message) => {
    throw new Error(message);
  };
  db.createFunction(error);
  assertThrows(() => error("Boom!"), (err) => {
    assertInstanceOf(err, Error);
    assertEquals(err.message, "Boom!");
  });
  assertThrows(() => db.query("SELECT error('Boom!')"), (err) => {
    assertInstanceOf(err, SqliteError);
...
test("can have multiple functions at the same time", function () {
  const first = () => 1;
  const second = () => 2;
  const db = new DB();
...
test("expects correct argument count", function () {
...
test("variadic functions", function () {
...
test("accept and return bigint values", function () {
  assertEquals(
...
test("return void returns NULL", function () {
  const empty = () => {};
  const db = new DB();
...
test("return undefined returns NULL", function () {
  const nothing = (shouldReturnNull) =>
...
test("return date returns formatted string", function () {
  const unix = (unix) => new Date(unix);
  const db = new DB();
